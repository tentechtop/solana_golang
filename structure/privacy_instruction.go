...
// PrivacyInstructionStrategy 执行隐私固定指令 + 当前不依赖 VM 但保留程序边界。
type PrivacyInstructionStrategy struct {
	Program PublicKey
}

// ProgramID 返回隐私程序 ID + 供策略注册表索引。
func (strategy PrivacyInstructionStrategy) ProgramID() PublicKey {
	return strategy.Program
}

// Execute 执行隐私指令 + 按类型路由到固定状态转换。
func (strategy PrivacyInstructionStrategy) Execute(context InstructionExecutionContext) error {
	instruction, err := UnmarshalPrivacyInstructionBinary(context.Instruction.Data)
	if err != nil {
		return err
	}
	switch instruction.Type {
	case PrivacyInstructionDeposit:
		return executePrivacyDeposit(strategy.Program, instruction, context)
	case PrivacyInstructionWithdraw:
		return executePrivacyWithdraw(strategy.Program, instruction, context)
	case PrivacyInstructionTransfer:
		return executePrivacyTransfer(strategy.Program, instruction, context)
	case PrivacyInstructionAuthorizeAudit:
		return executePrivacyAuthorizeAudit(strategy.Program, instruction, context)
	default:
		return fmt.Errorf("%w: unsupported privacy type %d", ErrInvalidPrivacyInstruction, instruction.Type)
	}
}
...
func executePrivacyDeposit(programID PublicKey, instruction PrivacyInstruction, context InstructionExecutionContext) error {
	if len(context.Instruction.AccountIndexes) < 2 {
		return fmt.Errorf("%w: deposit requires source and privacy state", ErrInvalidPrivacyInstruction)
	}
	...
	if !isSignerAddress(sourceAddress, context.Message) {
		return fmt.Errorf("%w: privacy deposit source must sign", ErrMissingRequiredSignature)
	}
	state, stateAccount, err := loadPrivacyStateAccount(programID, stateAddress, context.Accounts)
	if err != nil {
		return err
	}
	if state.findNote(instruction.Deposit.Commitment) >= 0 {
		return fmt.Errorf("%w: duplicate privacy commitment", ErrInvalidPrivacyInstruction)
	}
	if err := validatePrivacyAuditRecordsForSlot(instruction.Deposit.AuditRecords, context.CurrentSlot); err != nil {
		return err
	}
	if err := transferLamports(sourceAddress, stateAddress, instruction.Deposit.Amount, context.Accounts, context.RentConfig); err != nil {
		return err
	}

	stateAccount = context.Accounts[stateAddress].Clone()
	state.Notes = append(state.Notes, PrivacyNoteRecord{
		...
		AuditRecords:   clonePrivacyAuditRecords(instruction.Deposit.AuditRecords),
	})
	return storePrivacyStateAccount(stateAddress, stateAccount, state, context)
}

func executePrivacyWithdraw(programID PublicKey, instruction PrivacyInstruction, context InstructionExecutionContext) error {
	if len(context.Instruction.AccountIndexes) < 2 {
		return fmt.Errorf("%w: withdraw requires privacy state and destination", ErrInvalidPrivacyInstruction)
	}
	...
	if err := validatePrivacyAuditRecordsForSlot(instruction.Withdraw.AuditRecords, context.CurrentSlot); err != nil {
		return err
	}
	proofMessage, err := BuildPrivacyWithdrawProofMessage(instruction.VMVersion, stateAddress, destinationAddress, *instruction.Withdraw, context.CurrentSlot)
	if err != nil {
		return err
	}
	if err := state.consumeNote(instruction.Withdraw.SourceCommitment, instruction.Withdraw.Nullifier, instruction.Withdraw.Amount, context.CurrentSlot, context.Message, instruction.Proof, proofMessage); err != nil {
		return err
	}
	if err := state.appendAuditRecords(instruction.Withdraw.SourceCommitment, instruction.Withdraw.AuditRecords); err != nil {
		return err
	}
	if err := storePrivacyStateAccount(stateAddress, stateAccount, state, context); err != nil {
		return err
	}
	return transferLamports(stateAddress, destinationAddress, instruction.Withdraw.Amount, context.Accounts, context.RentConfig)
}

func executePrivacyTransfer(programID PublicKey, instruction PrivacyInstruction, context InstructionExecutionContext) error {
	if len(context.Instruction.AccountIndexes) < 1 {
		return fmt.Errorf("%w: private transfer requires privacy state", ErrInvalidPrivacyInstruction)
	}
	stateAddress := context.Message.AccountKeys[context.Instruction.AccountIndexes[0]]
	state, stateAccount, err := loadPrivacyStateAccount(programID, stateAddress, context.Accounts)
	if err != nil {
	...
	if state.findNote(instruction.Transfer.OutputCommitment) >= 0 {
		return fmt.Errorf("%w: duplicate output commitment", ErrInvalidPrivacyInstruction)
	}
	if err := validatePrivacyAuditRecordsForSlot(instruction.Transfer.OutputAuditRecords, context.CurrentSlot); err != nil {
		return err
	}
	proofMessage, err := BuildPrivacyTransferProofMessage(instruction.VMVersion, stateAddress, *instruction.Transfer, context.CurrentSlot)
	if err != nil {
		return err
	}
	if err := state.consumeNote(instruction.Transfer.SourceCommitment, instruction.Transfer.Nullifier, instruction.Transfer.Amount, context.CurrentSlot, context.Message, instruction.Proof, proofMessage); err != nil {
		return err
	}
	state.Notes = append(state.Notes, PrivacyNoteRecord{
		...
		AuditRecords:   clonePrivacyAuditRecords(instruction.Transfer.OutputAuditRecords),
	})
	return storePrivacyStateAccount(stateAddress, stateAccount, state, context)
}

func executePrivacyAuthorizeAudit(programID PublicKey, instruction PrivacyInstruction, context InstructionExecutionContext) error {
	if len(context.Instruction.AccountIndexes) < 1 {
		return fmt.Errorf("%w: authorize audit requires privacy state", ErrInvalidPrivacyInstruction)
	}
	stateAddress := context.Message.AccountKeys[context.Instruction.AccountIndexes[0]]
	state, stateAccount, err := loadPrivacyStateAccount(programID, stateAddress, context.Accounts)
	if err != nil {
	...
	if err := validatePrivacyAuditRecordForSlot(auditRecord, context.CurrentSlot); err != nil {
		return err
	}
	if err := state.authorizeAudit(instruction.AuthorizeAudit.Commitment, auditRecord, context.Message); err != nil {
		return err
	}
	return storePrivacyStateAccount(stateAddress, stateAccount, state, context)
}

func loadPrivacyStateAccount(programID PublicKey, stateAddress PublicKey, accounts map[PublicKey]Account) (PrivacyState, Account, error) {
	account, exists := accounts[stateAddress]
	if !exists {
		return PrivacyState{}, Account{}, fmt.Errorf("%w: privacy state account not found", ErrInvalidLoadedTransaction)
	}
	if account.Owner != programID {
		return PrivacyState{}, Account{}, fmt.Errorf("%w: privacy state owner mismatch", ErrInvalidPrivacyInstruction)
	}
	state, err := UnmarshalPrivacyStateBinary(account.Data)
	if err != nil {
		return PrivacyState{}, Account{}, err
	}
	return state, account.Clone(), nil
}

func storePrivacyStateAccount(stateAddress PublicKey, account Account, state PrivacyState, context InstructionExecutionContext) error {
...
func (state PrivacyState) findNote(commitment Hash) int {
...
func (state PrivacyState) hasNullifier(nullifier Hash) bool {
...
func (state *PrivacyState) consumeNote(commitment Hash, nullifier Hash, amount uint64, currentSlot uint64, message ResolvedMessage, proof []byte, proofMessage []byte) error {
	if currentSlot == 0 {
		return fmt.Errorf("%w: spend slot cannot be zero", ErrInvalidPrivacyInstruction)
	}
	if state.hasNullifier(nullifier) {
		return fmt.Errorf("%w: nullifier already spent", ErrInvalidPrivacyInstruction)
	}
	noteIndex := state.findNote(commitment)
	if noteIndex < 0 {
		return fmt.Errorf("%w: source commitment not found", ErrInvalidPrivacyInstruction)
	}
	note := state.Notes[noteIndex]
	if note.Spent {
		return fmt.Errorf("%w: source commitment already spent", ErrInvalidPrivacyInstruction)
	}
	if note.Amount != amount {
		return fmt.Errorf("%w: privacy amount mismatch", ErrInvalidPrivacyInstruction)
	}
	if err := validatePrivacySpendAuthorization(note.SpendAuthority, message, proof, proofMessage); err != nil {
		return err
	}
	state.Notes[noteIndex].Spent = true
	state.Notes[noteIndex].SpentSlot = currentSlot
	state.Notes[noteIndex].SpendNullifier = nullifier
	state.SpentNullifiers = append(state.SpentNullifiers, nullifier)
	return nil
}

func validatePrivacySpendAuthorization(spendAuthority PublicKey, message ResolvedMessage, proof []byte, proofMessage []byte) error {
	if isSignerAddress(spendAuthority, message) {
		return nil
	}
	if len(proof) == 0 {
		return fmt.Errorf("%w: spend authority must sign or provide zk proof", ErrMissingRequiredSignature)
	}
	if len(proofMessage) == 0 {
		return fmt.Errorf("%w: spend proof message is empty", ErrInvalidPrivacyInstruction)
	}
	if err := zk.VerifySchnorrProofBytes(proof, proofMessage, zk.Digest(spendAuthority)); err != nil {
		return fmt.Errorf("%w: verify spend zk proof: %w", ErrInvalidPrivacyInstruction, err)
	}
	return nil
}

func (state *PrivacyState) appendAuditRecords(commitment Hash, records []PrivacyAuditRecord) error {
	if len(records) == 0 {
		return nil
	}
	noteIndex := state.findNote(commitment)
	if noteIndex < 0 {
		return fmt.Errorf("%w: audit commitment not found", ErrInvalidPrivacyInstruction)
	}
	for _, record := range records {
		if hasPrivacyAuditRecord(state.Notes[noteIndex].AuditRecords, record) {
			return fmt.Errorf("%w: duplicate audit authorization", ErrInvalidPrivacyInstruction)
		}
		if len(state.Notes[noteIndex].AuditRecords) >= MaxPrivacyAuditRecordsPerNote {
			return fmt.Errorf("%w: audit record count exceeds %d", ErrInvalidPrivacyInstruction, MaxPrivacyAuditRecordsPerNote)
		}
		state.Notes[noteIndex].AuditRecords = append(state.Notes[noteIndex].AuditRecords, clonePrivacyAuditRecord(record))
	}
	return nil
}

func (state *PrivacyState) authorizeAudit(commitment Hash, record PrivacyAuditRecord, message ResolvedMessage) error {
	noteIndex := state.findNote(commitment)
	if noteIndex < 0 {
		return fmt.Errorf("%w: audit commitment not found", ErrInvalidPrivacyInstruction)
	}
	note := state.Notes[noteIndex]
	if note.Spent {
		return fmt.Errorf("%w: cannot authorize spent note", ErrInvalidPrivacyInstruction)
	}
	if !isSignerAddress(note.SpendAuthority, message) {
		return fmt.Errorf("%w: audit authorization authority must sign", ErrMissingRequiredSignature)
	}
	if hasPrivacyAuditRecord(note.AuditRecords, record) {
		return fmt.Errorf("%w: duplicate audit authorization", ErrInvalidPrivacyInstruction)
	}
	if len(note.AuditRecords) >= MaxPrivacyAuditRecordsPerNote {
		return fmt.Errorf("%w: audit record count exceeds %d", ErrInvalidPrivacyInstruction, MaxPrivacyAuditRecordsPerNote)
	}
	state.Notes[noteIndex].AuditRecords = append(state.Notes[noteIndex].AuditRecords, clonePrivacyAuditRecord(record))
	return nil
