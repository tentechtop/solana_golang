...
func normalizeNodeConfig(config nodeConfig) (nodeConfig, error) {
	...
	privacyExecutionMode, err := runtimepkg.NormalizePrivacyExecutionMode(config.PrivacyExecutionMode)
	if err != nil {
		return nodeConfig{}, fmt.Errorf("posnode: invalid privacy execution mode: %w", err)
	}
	config.PrivacyExecutionMode = privacyExecutionMode
